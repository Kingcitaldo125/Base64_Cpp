...
std::string base64decode(const std::string &conv) {
  ...
  for (long unsigned int index = 0; index < conv.size(); index += 4) {
    for (long unsigned int encoded_idx = index; encoded_idx < index + 4;
         ++encoded_idx) {
  ...
  // Convert the chunks to bytes and write out the decoded byte to the
  // stringstream return after 'all' bytes were written out to the stream Don't
  // write out padding bytes to the stream
  std::stringstream ss;
